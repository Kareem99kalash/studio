...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build coordinates: stores + polygon centers
    const coords = [
      ...stores.map((s: any) => [s.lng, s.lat]),
      ...polygons.map((p: any) => [p.lng, p.lat])
    ];

    // Build query string - sources are first N (stores), destinations are last M (polygons)
    const srcIndices = stores.map((_: any, i: number) => i).join(';');
    const dstIndices = polygons.map((_: any, i: number) => stores.length + i).join(';');
