...
if (!secretString) {
  console.warn('JWT_SECRET is not set. Token operations will fail at runtime if not configured on the server.');
}
const SECRET_KEY = new TextEncoder().encode(secretString || 'fallback_secret_for_build_only');
